<template>
    ...
    <v-card :elevation="hover ? 16:2" :class="{'on-hover' : hover}">
        ...
        <v-row align="center" class="mx-0">
            <v-rating :value="movie.vote_average / 2" color="amber" dense half-increments readonly size="18"></v-rating>
            <div>
        ...
        <div class="my-4 subtitle-2">
            ...
            {{ genredTypeName(genre, index) }}</span>
        </div>
     </v-card>
    </v-hover>
</template>
...
export default {
    ...
    methods : {
        genredTypeName(genraId, index){
            for (const item of this.genres){
                if(item.id == genraId){
                    if(this.movie.genre_ids.lenght - 1 == index){
                        return item.name
                    }else{
                        return item.name + ",";
                    }
