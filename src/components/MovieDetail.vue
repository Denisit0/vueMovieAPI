    ...
    <div>
        ...
        <v-row>
            <v-col cols="12" sm="4">
                ...
                <v-col cols="12" sm="8">
                    <h1 class="red--text text-darken-3 mt-5">{{ this.movie.title }}</h1>
                    <v-row>
                        <v-col cols="12" sm="2">
                            <v-rating
                        ...
                        <v-col cols="12" sm="7">
                            <div class="subtitle-2 grey--text ml-n16">
                                <span v-for="(item, index) in movie.genres" :key="index" class="ml-1">
                                    {{ item.name }} 
                                    <span v-if="(movie.genres.lenght -1 != index)">,</span>
                                </span>
                            </div>
                        </v-col>
                    </v-row>
                    <p class="mt-5 gey--text text--darken-3 subheader">{{ this.movie.overview }}</p>
                    <div class="mt-5">
                        <h2 class="mt-5 grey--text text--darken-3">Elenco destacado</h2>
                    ...
                    <template v-slot:activator="{ on, attrs }">
                       <v-btn tile color="success" v-bind="attrs" v-on="on" @click.prevent="openYouTubeModel">
                                ...
                                <v-row>
                                    <v-col cols="12" sm="">
                                        <div class="iframe-container">
                                            ...
                                            <iframe :src="mediaURL" allowfullscreen v-if="isVideo"></iframe>
                                        </div>
                                    </v-col>
                                </v-row>
                            </v-container>
       ...
       <CastNames :casts="movie.credits.casts"></CastNames>
       <v-divider class="mt-2"></v-divider>
       <ImageAct :images="movie.images.backdrops"></ImageAct>
        </v-container>
    </div>
</template>

<script>
import ImageAct from '../components/ImageAct.vue';
import CastNames from '../components/CastNames.vue';
export default{
    components:{
        CastNames,
        ImageAct,
